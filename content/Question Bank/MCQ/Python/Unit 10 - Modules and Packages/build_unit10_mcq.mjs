...
const UNIT = 10;
const TITLE = "Modules and Packages";
const TOPIC = "modules-and-packages";
const SHEET_NAME = "Python - MCQ - 10";
const UNIT_DIR = path.join(
  ROOT,
  "content/Question Bank/MCQ/Python/Unit 10 - Modules and Packages",
);
const DATA_FILE = path.join(UNIT_DIR, "unit10_questions.json");
const TEMPLATE_FILE = path.join(
  ROOT,
  "content/Question Bank/Template/questions-mcq-template.xlsx",
);
const CONTENT_FILE = path.join(UNIT_DIR, "Unit 10 - Modules and Packages - MCQ.xlsx");
const DELIVERY_DIR = path.join(ROOT, "outputs/units-6-13-qc-upgrade");
const DELIVERY_FILE = path.join(DELIVERY_DIR, "Unit 10 - Modules and Packages - MCQ.xlsx");
const PREVIEW_FILE = path.join(DELIVERY_DIR, "Unit 10 - Modules and Packages - MCQ.png");
...
function validateQuestions(questions) {
  ...
  assert(new Set(questions.slice(0, 10).map((question) => question.subtopic)).size === 2, "Set 1 must cover both Unit 10 taxonomy subtopics");
