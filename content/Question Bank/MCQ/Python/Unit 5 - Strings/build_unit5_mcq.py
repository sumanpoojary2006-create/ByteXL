"""Build the benchmark-quality Unit 5 - Strings MCQ workbook.

Question content lives in ``unit5_questions.json`` and is grounded only in the
Unit 5 reading materials. Set 1 contains 10 course MCQs; Set 2 contains 30
...
DATA_FILE = os.path.join(OUT_DIR, "unit5_questions.json")
...
def build_rows(questions):
    ...
    for item in questions:
        ...
        rows.append([
            f"Python - MCQ - 5.{set_number}.{counters[set_number]}",
            item["description"],
            item["explanation"],
