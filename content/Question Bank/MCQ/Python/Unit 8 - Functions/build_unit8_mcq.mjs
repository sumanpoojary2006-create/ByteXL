...
const SHEET_NAME = "Python - MCQ - 8";
const UNIT_DIR = path.join(
  ROOT,
  "content/Question Bank/MCQ/Python/Unit 8 - Functions",
);
const DATA_FILE = path.join(UNIT_DIR, "unit8_questions.json");
const TEMPLATE_FILE = path.join(
  ROOT,
  "content/Question Bank/Template/questions-mcq-template.xlsx",
);
const CONTENT_FILE = path.join(UNIT_DIR, "Unit 8 - Functions - MCQ.xlsx");
const DELIVERY_DIR = path.join(ROOT, "outputs/units-6-13-qc-upgrade");
const DELIVERY_FILE = path.join(DELIVERY_DIR, "Unit 8 - Functions - MCQ.xlsx");
const PREVIEW_FILE = path.join(DELIVERY_DIR, "Unit 8 - Functions - MCQ.png");
...
function validateQuestions(questions) {
  const allowedSubtopics = new Set([
    "defining-and-calling-functions", "docstrings-and-clean-code",
    "function-parameters-and-arguments", "higher-order-functions",
    "lambda-functions", "recursion", "variable-scope-and-legb",
  ]);
  ...
  assert(new Set(questions.slice(0, 10).map((question) => question.subtopic)).size === 7, "Set 1 must cover all seven Unit 8 taxonomy subtopics");
