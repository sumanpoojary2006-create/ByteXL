...
const UNIT = 13;
const TITLE = "Debugging";
const TOPIC = "debugging";
const SHEET_NAME = "Python - MCQ - 13";
const UNIT_DIR = path.join(
  ROOT,
  "content/Question Bank/MCQ/Python/Unit 13 - Debugging",
);
const DATA_FILE = path.join(UNIT_DIR, "unit13_questions.json");
const TEMPLATE_FILE = path.join(
  ROOT,
  "content/Question Bank/Template/questions-mcq-template.xlsx",
);
const CONTENT_FILE = path.join(UNIT_DIR, "Unit 13 - Debugging - MCQ.xlsx");
const DELIVERY_DIR = path.join(ROOT, "outputs/units-6-13-qc-upgrade");
const DELIVERY_FILE = path.join(DELIVERY_DIR, "Unit 13 - Debugging - MCQ.xlsx");
const PREVIEW_FILE = path.join(DELIVERY_DIR, "Unit 13 - Debugging - MCQ.png");
...
function validateQuestions(questions) {
  const allowedSubtopics = new Set([
    "assertions", "debugging-techniques", "debugging-workflow", "logging",
  ]);
  ...
  assert(new Set(questions.slice(0, 10).map((question) => question.subtopic)).size === 4, "Set 1 must cover all four Unit 13 taxonomy subtopics");
