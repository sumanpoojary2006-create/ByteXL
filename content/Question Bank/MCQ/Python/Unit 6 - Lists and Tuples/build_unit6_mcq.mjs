...
const UNIT = 6;
const TITLE = "Lists and Tuples";
const TOPIC = "lists-and-tuples";
const SHEET_NAME = "Python - MCQ - 6";
const UNIT_DIR = path.join(
  ROOT,
  "content/Question Bank/MCQ/Python/Unit 6 - Lists and Tuples",
);
const DATA_FILE = path.join(UNIT_DIR, "unit6_questions.json");
const TEMPLATE_FILE = path.join(
  ROOT,
  "content/Question Bank/Template/questions-mcq-template.xlsx",
);
const CONTENT_FILE = path.join(UNIT_DIR, "Unit 6 - Lists and Tuples - MCQ.xlsx");
const DELIVERY_DIR = path.join(ROOT, "outputs/units-6-13-qc-upgrade");
const DELIVERY_FILE = path.join(DELIVERY_DIR, "Unit 6 - Lists and Tuples - MCQ.xlsx");
const PREVIEW_FILE = path.join(DELIVERY_DIR, "Unit 6 - Lists and Tuples - MCQ.png");
...
function validateQuestions(questions) {
  ...
  const difficultyCounts = tally(questions.map((question) => question.difficulty));
  assert(JSON.stringify(difficultyCounts) === JSON.stringify({ easy: 10, hard: 10, medium: 20 }), `Difficulty balance failed: ${JSON.stringify(difficultyCounts)}`);
  assert(new Set(questions.slice(0, 10).map((question) => question.difficulty)).size === 3, "Set 1 must contain easy, medium, and hard questions");
  assert(new Set(questions.slice(10).map((question) => question.difficulty)).size === 3, "Set 2 must contain easy, medium, and hard questions");
  assert(new Set(questions.map((question) => question.subtopic)).size === 8, "Unit 6 must cover all eight taxonomy subtopics");
  ...
  assert(maximumRun === 1, `Answer letter repeats ${maximumRun} times consecutively`);

  let correctIsLongest = 0;
  let correctIsShortest = 0;
  for (const question of questions) {
    const lengths = question.options.map((option) => option.replace(/```\w*|```|`/g, "").length);
  ...
  assert(correctIsLongest === 0, `Correct option is longest in ${correctIsLongest}/40 questions`);

  return { answerCounts, set1Counts, difficultyCounts, maximumRun, correctIsLongest, correctIsShortest };
}
