"""Build the benchmark-quality Unit 2 MCQ workbook from canonical JSON."""
...
OUT_DIR = "content/Question Bank/MCQ/Python/Unit 2 - Data Types and Operators"
OUT_FILE = os.path.join(OUT_DIR, "Unit 2 - Data Types and Operators - MCQ.xlsx")
DATA_FILE = os.path.join(OUT_DIR, "unit2_questions.json")
SHEET_NAME = "Python - MCQ - 2"
...
for item in questions:
    counters[item["set"]] += 1
    rows.append([
        f"Python - MCQ - 2.{item['set']}.{counters[item['set']]}",
        item["description"], item["explanation"], SCORE[item["difficulty"]],
        "published", item["difficulty"], item["bloom"],
        f"python - Set {item['set']}", "python", "data-types-and-operators",
        item["subtopic"], None, *item["options"], LETTER_TO_NUMBER[item["answer"]],
    ])
