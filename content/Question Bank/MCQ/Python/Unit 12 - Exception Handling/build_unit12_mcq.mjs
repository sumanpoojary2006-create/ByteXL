...
const UNIT = 12;
const TITLE = "Exception Handling";
const TOPIC = "exception-handling";
const SHEET_NAME = "Python - MCQ - 12";
const UNIT_DIR = path.join(
  ROOT,
  "content/Question Bank/MCQ/Python/Unit 12 - Exception Handling",
);
const DATA_FILE = path.join(UNIT_DIR, "unit12_questions.json");
const TEMPLATE_FILE = path.join(
  ROOT,
  "content/Question Bank/Template/questions-mcq-template.xlsx",
);
const CONTENT_FILE = path.join(UNIT_DIR, "Unit 12 - Exception Handling - MCQ.xlsx");
const DELIVERY_DIR = path.join(ROOT, "outputs/units-6-13-qc-upgrade");
const DELIVERY_FILE = path.join(DELIVERY_DIR, "Unit 12 - Exception Handling - MCQ.xlsx");
const PREVIEW_FILE = path.join(DELIVERY_DIR, "Unit 12 - Exception Handling - MCQ.png");
...
function validateQuestions(questions) {
  const allowedSubtopics = new Set([
    "custom-exceptions", "defensive-programming",
    "exception-handling-patterns", "raising-exceptions",
    "syntax-runtime-and-logic-errors", "traceback-analysis",
    "try-except",
  ]);
  ...
  assert(new Set(questions.slice(0, 10).map((question) => question.subtopic)).size === 7, "Set 1 must cover all seven Unit 12 taxonomy subtopics");
