...
const UNIT = 7;
const TITLE = "Sets and Dictionaries";
const TOPIC = "sets-and-dictionaries";
const SHEET_NAME = "Python - MCQ - 7";
const UNIT_DIR = path.join(
  ROOT,
  "content/Question Bank/MCQ/Python/Unit 7 - Sets and Dictionaries",
);
const DATA_FILE = path.join(UNIT_DIR, "unit7_questions.json");
const TEMPLATE_FILE = path.join(
  ROOT,
  "content/Question Bank/Template/questions-mcq-template.xlsx",
);
const CONTENT_FILE = path.join(UNIT_DIR, "Unit 7 - Sets and Dictionaries - MCQ.xlsx");
const DELIVERY_DIR = path.join(ROOT, "outputs/units-6-13-qc-upgrade");
const DELIVERY_FILE = path.join(DELIVERY_DIR, "Unit 7 - Sets and Dictionaries - MCQ.xlsx");
const PREVIEW_FILE = path.join(DELIVERY_DIR, "Unit 7 - Sets and Dictionaries - MCQ.png");
...
function validateQuestions(questions) {
  const allowedSubtopics = new Set([
    "sets", "set-operations", "dictionaries", "dictionary-operations",
    "dictionary-comprehensions", "nested-dictionaries", "choosing-data-structures",
  ]);
  ...
  assert(new Set(questions.slice(0, 10).map((question) => question.subtopic)).size === 7, "Set 1 must cover all seven Unit 7 taxonomy subtopics");
