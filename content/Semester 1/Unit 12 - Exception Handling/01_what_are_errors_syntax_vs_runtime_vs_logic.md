...
```python
# The code below is missing the colon after the condition.
# We feed it to compile() so Python reports the SyntaxError as output
# instead of refusing to run this whole block:
broken_code = '''if age >= 18
    print("Eligible")'''

try:
    compile(broken_code, "signup.py", "exec")
except SyntaxError as e:
    print(f"SyntaxError: {e.msg}")
    print(f"  on line {e.lineno}: {e.text.strip()}")
    print("Python refuses to run any of the file until the grammar is fixed.")
```
...
print(average(10, 20))    # 20.0, not the intended 15.0
```
...
# A) SyntaxError -- Python detects this before any code runs
try:
    ast.parse('print("Hello"')    # missing closing parenthesis
except SyntaxError as e:
    print(f"A) SyntaxError (caught at parse time): {e}")

# B) RuntimeError -- runs, then crashes at this specific line
try:
    result = 10 / 0
except ZeroDivisionError as e:
    print(f"B) ZeroDivisionError (raised at runtime): {e}")

# C) Logic error -- no crash, but the name misleads; n + n is addition, not multiplication
def double(n): return n + n
print(f"C) double(5) = {double(5)}  -- correct here, but a logic error if intent was n * 2")
```
