...
try:
    ...
    # Print the full traceback as output, so we can read it bottom-up:
...
def split_into_groups(people, group_size):
    return people / group_size

def plan_seating(total_people):
    return split_into_groups(total_people, 0)

try:
    plan_seating(40)
except ZeroDivisionError as e:
    print(f"ZeroDivisionError: {e}")
    print("Traceback (innermost call last): split_into_groups → plan_seating → call site")
```
