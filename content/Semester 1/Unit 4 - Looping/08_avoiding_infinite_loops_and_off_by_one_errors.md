...
```python
count = 1
safety = 0    # only here so this demo can stop; a real infinite loop has no such guard
while count <= 5:
    print(count)
    # forgot to update count -- count never changes, so count <= 5 stays True forever
    safety += 1
    if safety >= 10:
        print("... this would go on forever. Stopping the demo after 10 prints.")
        break
```
...
print(total)    # what does this actually add up to?
```
