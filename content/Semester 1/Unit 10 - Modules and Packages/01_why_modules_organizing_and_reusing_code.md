...
```python
def split_cost(total, people): return total / people
def add_service_charge(total, rate): return total * (1 + rate)
def format_receipt(item, amount): return f"{item}: Rs.{amount:.2f}"
def days_until_trip(trip_date): return None    # needs a real date in production
def random_room_assignment(names): return None # needs random logic in production

# Which belong together? split_cost + add_service_charge = billing module
#                        format_receipt alone could be a formatting module
#                        days_until_trip + random_room_assignment = trip module
print(f"split_cost(1200, 4) -> {split_cost(1200, 4)}")
print(f"add_service_charge(1000, 0.10) -> {add_service_charge(1000, 0.10)}")
print(f"format_receipt('T-shirt', 350) -> {format_receipt('T-shirt', 350)}")
```
