...
print(fest_stalls["Food"])             # {'Samosa': 30, 'Cold Drink': 40}
print(fest_stalls["Food"]["Samosa"])   # 30
print(fest_stalls["Games"]["Dart"])    # 25
```
...
print(fest_stalls["Food"])    # {'Samosa': 35, 'Cold Drink': 40}
```
...
fest_stalls["Books"] = {"Novel": 120, "Comic": 60}
print(fest_stalls.keys())    # dict_keys(['Merch', 'Food', 'Games', 'Books'])
```
...
```python
stalls = {}
for stall_name in ["Merch", "Food"]:
    items = {}
    for _ in range(2):
        name = input(f"{stall_name} item name: ")
        price = float(input(f"{name} price: "))
        items[name] = price
    stalls[stall_name] = items
