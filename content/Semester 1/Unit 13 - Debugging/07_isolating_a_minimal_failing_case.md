...
```python
pipeline = ["CSV file", "loader function", "Attendee objects", "Event", "summary"]
print("Full pipeline: " + " -> ".join(pipeline))
print("Somewhere in there, one count is wrong. Where, specifically?")
print("Debugging the whole chain at once is slow -- minimization finds the exact step.")
```
...
attendees = [Attendee("Zara", 2), Attendee("Lin", "3")]
try:
    print(total_rsvps(attendees))    # error!
except TypeError:
...
```python
print("Before isolating: 'the total is wrong somewhere in this whole project'")
print("After isolating:  'the CSV loader forgot to call int() on one column'")
print()
print("Minimization turns a vague, intimidating bug")
print("into a small, specific, answerable question.")
```
...
cart = Cart([100, 200, "50"])
try:
    print(discounted_total(cart.prices, 10))    # error!
except TypeError:
    print(traceback.format_exc())
```
