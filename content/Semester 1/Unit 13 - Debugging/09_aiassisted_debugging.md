...
```python
weak_prompt = "fix my code [pastes the entire four-file project]"

strong_prompt = (
    "total_rsvps() raises TypeError: unsupported operand type(s) for +: 'int' and 'str'.\n"
    "Minimal case: counts = [2, '3']; sum(counts) -> same error.\n"
    "Expected: all values are ints. Actual: one is a string.\n"
    "Theory: my CSV loader forgets to call int() on one column.\n"
    "Can you confirm and suggest a fix for load_attendees()?"
)

print("Weak AI prompt:")
print(" ", weak_prompt)
print()
print("Strong AI prompt (specific, self-contained, testable):")
for line in strong_prompt.split("\n"):
    print(" ", line)
```
...
def load_attendees(rows):
    attendees = []
    for name, raw_count in rows:
        count = int(raw_count)    # the suggested fix: convert explicitly here
        attendees.append((name, count))
    return attendees

rows = [("Zara", "2"), ("Lin", "3")]
print(load_attendees(rows))
```
...
def load_attendees(rows):
    attendees = []
    for name, raw_count in rows:
        count = int(raw_count)    # the fix from the previous step
        attendees.append((name, count))
    return attendees

def total_rsvps(attendees):
    return sum(count for _name, count in attendees)

fixed_attendees = load_attendees([("Zara", "2"), ("Lin", "3")])
print(total_rsvps(fixed_attendees))    # 5, the bug is genuinely gone
```
...
```python
ai_uses = [
    ("Good use",  "Explain what this traceback means -> you still verify the explanation"),
    ("Good use",  "Here's my minimal case and theory; does this fix make sense?"),
    ("Risky use", "Paste the fix immediately, no reading, no understanding, no rerun"),
]

print(f"{'Category':<12} {'Description'}")
print("-" * 65)
for category, description in ai_uses:
    print(f"{category:<12} {description}")
```
...
try:
    print(average_rating([]))    # error! empty list -> divide by zero
