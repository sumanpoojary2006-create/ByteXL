...
def total_rsvps(attendees):
    ...
    for attendee in attendees:
        # breakpoint()   <-- in a real terminal, pause here and inspect the values.
        # onecompiler cannot run the interactive (Pdb) prompt, so we print the
        # same state the debugger would let you look at:
        print(f"  [paused] attendee={attendee.name!r}, rsvp_count={attendee.rsvp_count}, total so far={total}")
        total += attendee.rsvp_count
    return total

result = total_rsvps([Attendee("Zara", 2), Attendee("Lin", 3), Attendee("Omar", 4)])
print(f"total_rsvps(...) -> {result}")
```
...
def average_rsvps(attendees):
    ...
    count = 0
    for attendee in attendees:
        # breakpoint()   <-- pause here in a real terminal and inspect the state.
        # Here we print what the (Pdb) prompt would let you check by hand:
        print(f"  [paused] name={attendee.name!r}, total={total}, count={count}")
        total += attendee.rsvp_count
        count += 1
    return total / count

result = average_rsvps([Attendee("Zara", 2), Attendee("Lin", 3)])
print(f"average_rsvps(...) -> {result}")
```
