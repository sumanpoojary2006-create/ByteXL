...
Keep every `CREATE TABLE` and `INSERT` statement in `init.sql`. Keep only the task query currently being worked on in a separate SQL file so the setup code and task work remain easy to review.
