...
Confirm that `idx_orders_amount` is used for a range query, then check the plan for finding the single largest `amount` in the table, and note in a comment whether the B-tree helps with that too.

```postgresql with=btree_demo.sql
-- Write your queries and comment below
```

`EXPLAIN SELECT * FROM orders WHERE amount > 90000.00;` and `EXPLAIN SELECT MAX(amount) FROM orders;` both show the planner using `idx_orders_amount`, since the largest value sits at one end of the sorted leaf level, letting the database jump straight there instead of scanning and comparing every row to find the maximum.
