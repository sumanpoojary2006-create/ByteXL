...
```postgresql with=storage_demo.sql
SELECT COUNT(*) AS rows_needed, pg_size_pretty(pg_relation_size('orders')) AS full_table_size
FROM orders
WHERE order_id = 1;
```

This query logically needs exactly one row, but physically, the database still has to fetch the entire page that row lives on, discarding the rest of that page's rows only after they have already been read into memory. This is the foundational fact behind why the next lessons in this unit matter so much:
