...
```console
# A typical library CLI workflow:
library-cli export-overdue --branch main --format csv > overdue.csv
library-cli notify-patrons --input overdue.csv --template email
library-cli report daily --date 2026-07-01 | grep "late return"
```
...
# Good: output goes to stdout, error goes to stderr
if not found:
    print("Error: book not found", file=sys.stderr)
    sys.exit(1)   # non-zero: failure

print(book.title)   # stdout: goes to next pipe stage
