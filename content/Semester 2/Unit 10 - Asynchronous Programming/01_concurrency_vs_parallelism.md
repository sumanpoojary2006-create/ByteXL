...
# Simulate sequential HTTP calls with time.sleep
def mock_get(library_id, isbn):
    time.sleep(0.05)  # simulate 50ms network wait
    return {"library": library_id, "isbn": isbn, "available": True}

def check_availability_sequential(isbn):
    start = time.time()
    r1 = mock_get("library1", isbn)
    r2 = mock_get("library2", isbn)
    r3 = mock_get("library3", isbn)
    elapsed = time.time() - start
    return [r1, r2, r3], elapsed

results, total_time = check_availability_sequential("978-0-13-235088-4")
print(f"Sequential: {len(results)} libraries checked in {total_time:.3f}s")
print(f"Time = sum of all 3 waits (would be ~1500ms for 500ms each)")
for r in results:
    print(f"  {r['library']}: available={r['available']}")
```
...
# asyncio-based concurrent check (using asyncio.sleep to simulate network wait)
async def async_get(library_id, isbn):
    await asyncio.sleep(0.05)   # simulate 50ms network wait (non-blocking)
    return {"library": library_id, "isbn": isbn, "available": True}

async def check_availability_concurrent(isbn):
    tasks = [
        async_get("library1", isbn),
        async_get("library2", isbn),
        async_get("library3", isbn),
    ]
    responses = await asyncio.gather(*tasks)   # all three run concurrently
    return responses

import time
start = time.time()
results = asyncio.run(check_availability_concurrent("978-0-13-235088-4"))
elapsed = time.time() - start

print(f"Concurrent: {len(results)} libraries checked in {elapsed:.3f}s")
print(f"Time = max of all waits (not sum) -- async overlap saves time")
for r in results:
    print(f"  {r['library']}: available={r['available']}")
```
