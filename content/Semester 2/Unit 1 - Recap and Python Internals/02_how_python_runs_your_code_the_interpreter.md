...
# SyntaxError is caught at PARSE time -- before any code runs
broken_source = "def broken(: pass"   # invalid syntax

try:
    ast.parse(broken_source)
except SyntaxError as e:
    print(f"SyntaxError caught at parse time: {e}")
    print("Even line 1 of the file would NOT run -- parsing happens first.")

# NameError is caught at EXECUTION time (the parse stage sees no problem)
valid_source = "result = undefined_variable + 1"
tree = ast.parse(valid_source)
print(f"\nParsed successfully: {ast.dump(tree)[:60]}...")
print("NameError would only appear when Python EXECUTES this line.")
```
