...
# Print the first entry -- usually the script directory
print(sys.path[0])

# Show what PYTHONPATH contributes (if set)
import os
...
```python
# Show why sys.path.append is fragile:
import sys

approaches = {
    "sys.path.append('/absolute/path')":  "Fragile -- hardcoded path breaks on other machines",
    "pip install -e . (editable install)":"Best -- package is importable from anywhere in venv",
    "PYTHONPATH=. python main.py":        "OK for development, not for production",
}

print("Ways to make modules findable:")
for approach, verdict in approaches.items():
    print(f"\n  {approach}")
    print(f"  -> {verdict}")

print()
print(f"Current sys.path has {len(sys.path)} entries")
print(f"First entry (usually script dir): {sys.path[0]!r}")
print()
print("Avoid sys.path.append() in library code -- it has process-wide side effects.")
print("Prefer: pip install -e . or set PYTHONPATH environment variable.")
```
...
try:
    ...
    for p in sys.path:
        print(" ", p)
    print("my_library was not found in any of the above.")
```
