...
function convertMarkdown(markdown, filePath, settings) {
  ...
  for (const block of blocks) {
    ...
    if (attrs.with) {
      for (const name of attrs.with.split(",").map((item) => item.trim()).filter(Boolean)) {
        if (fileRegistry.has(name)) {
          if (SQL_LANGUAGES.has(decision.language)) {
    ...
    if (attrs.with && SQL_LANGUAGES.has(language)) {
      for (const name of attrs.with.split(",").map((item) => item.trim()).filter(Boolean)) {
        const contextCode = stripIncludedSetup(block.code, name);
        if (reusableSetupCode(language, contextCode)) {
...
function stripIncludedSetup(code, setupName) {
  const includePattern = new RegExp(`^\\s*\\\\i\\s+${escapeRegExp(setupName)}\\s*$`);
  return code
    .replace(/\n+$/, "")
    .split(/\n/)
    .filter((line) => !includePattern.test(line))
    .join("\n")
    .replace(/\n+$/, "");
}

function oneCompilerSqlCode(block, setupName, setup, context) {
  const parts = [setup.replace(/\n+$/, "")];
  const contextCode = context.map((item) => item.replace(/\n+$/, "")).filter(Boolean).join("\n\n");
  if (contextCode) {
    parts.push(`-- Reusable definitions from earlier examples\n${contextCode}`);
  }
  parts.push(`-- Query\n${stripIncludedSetup(block.code, setupName)}`);
  return parts.join("\n\n");
}

function reusableSetupCode(language, code) {
  if (language !== "postgresql") return false;
  return /\b(CREATE\s+(?:OR\s+REPLACE\s+)?(?:VIEW|MATERIALIZED\s+VIEW|FUNCTION|PROCEDURE|TRIGGER|ROLE)|PREPARE\s+\w+|CREATE\s+TABLE\s+schema_migrations\b)/is.test(code);
}
