...
def parse_fence_attrs(info: str) -> dict[str, str]:
    attrs: dict[str, str] = {}
    for token in (info or "").strip().split()[1:]:
        if "=" not in token:
            continue
...
def safe_display_filename(value: str) -> str | None:
    if not value:
        return None
...
def reusable_setup_code(language: str, code: str) -> bool:
    if language != "postgresql":
        return False
    return bool(
        re.search(
            r"(?is)\b("
            r"CREATE\s+(?:OR\s+REPLACE\s+)?(?:VIEW|MATERIALIZED\s+VIEW|FUNCTION|PROCEDURE|TRIGGER|ROLE)"
            r"|PREPARE\s+\w+"
            r"|CREATE\s+TABLE\s+schema_migrations\b"
            r")",
            code,
        )
    )
...
def strip_included_setup(block: FencedBlock, setup_name: str) -> str:
    query_lines = []
    include_pattern = re.compile(rf"^\s*\\i\s+{re.escape(setup_name)}\s*$")
...
def onecompiler_code(block: FencedBlock, setup_files: dict[str, str], reusable_context: dict[str, list[str]]) -> str:
    attrs = parse_fence_attrs(block.info)
    setup_name = attrs.get("with")
    if not setup_name or setup_name not in setup_files:
        return block.code.rstrip("\n")

    setup = setup_files[setup_name].rstrip("\n")
    context = "\n\n".join(item.rstrip("\n") for item in reusable_context.get(setup_name, []))
    query = strip_included_setup(block, setup_name)
    parts = [setup]
    if context:
        parts.append(f"-- Reusable definitions from earlier examples\n{context}")
...
def main() -> None:
    ...
    for md_path in markdown_files:
        ...
        reusable_context: dict[str, list[str]] = {}
        ...
        for block in blocks:
            ...
            file_block_index += 1
            converted_count += 1
            snippet_filename = filename_for(language, file_block_index)
            digest = hashlib.sha1(
                f"{rel_path.as_posix()}:{block.start_line}:{snippet_filename}:{snippet_code}".encode("utf-8")
            ).hexdigest()[:10]
            ...
            if "with" in attrs:
                setup_name = attrs["with"]
                context_code = strip_included_setup(block, setup_name)
                if reusable_setup_code(language, context_code):
                    reusable_context.setdefault(setup_name, []).append(context_code)
